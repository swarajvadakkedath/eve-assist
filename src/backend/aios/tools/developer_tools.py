...
def _next_cmd_id() -> str:
    global _cmd_counter
    _cmd_counter += 1
    return f"cmd_{_cmd_counter}_{datetime.utcnow().timestamp()}"
...
async def _run_command(params: dict, event_bus: EventBus | None = None) -> ToolResult:
    try:
        ...
        duration = (datetime.utcnow() - _running_commands[cmd_id]["started_at"]).total_seconds()
...
async def _stream_output(params: dict, event_bus: EventBus | None = None) -> ToolResult:
    try:
        ...
        _running_commands[cmd_id] = {
            "proc": proc, "command": command, "started_at": datetime.utcnow(),
    ...
    terminal_tools = [
        ToolContract(
            ...
            parameters={
                ...
                "shell": {"type": "boolean", "description": "Use shell to execute", "default": True},
            },
            returns={
