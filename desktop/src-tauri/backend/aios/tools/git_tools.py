...
def _next_op_id() -> str:
    global _op_counter
    _op_counter += 1
    return f"git_{_op_counter}_{datetime.now(timezone.utc).timestamp()}"
...
async def _run_git_streaming(
    ...
    if op_id:
        _running_operations[op_id] = {"proc": proc, "args": args, "started_at": datetime.now(timezone.utc), "status": "running"}
