...
class ClipboardProvider:
    ...
    async def collect(self) -> dict[str, Any]:
        ...
        try:
            ...
            text = stdout.decode("utf-8", errors="replace").strip() if proc.returncode == 0 else ""
...
class WorkspaceProvider:
    ...
    async def collect(self) -> dict[str, Any]:
        from aios.core.context.project_detector import detect_project_from_path, detect_project_from_file
        project = None
        if self._workspace_path:
            project = detect_project_from_path(self._workspace_path)
        if project is None and self._recent_files:
            project = detect_project_from_file(self._recent_files[0])
        project_dict = None
        if project:
            project_dict = {"path": project.path, "type": project.type, "markers": project.markers}
        return {
...
class GitProvider:
    ...
    async def collect(self) -> dict[str, Any]:
        ...
        try:
            ...
            is_dirty = bool(await _git(["status", "--porcelain"]))
            remote = await _git(["remote", "get-url", "origin"])
            self._branch = branch
